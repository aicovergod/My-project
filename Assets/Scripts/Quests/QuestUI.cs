...
namespace Quests
{
    ...
    public class QuestUI : MonoBehaviour
    {
        ...
        private void Refresh()
        {
            ...
            {
                var entry = Instantiate(questEntryPrefab, listContent);
                entry.name = quest.Title;

                var title = entry.transform.Find("Title")?.GetComponent<Text>();
                if (title != null)
                {
                ...
                var status = entry.transform.Find("Status")?.GetComponent<Text>();
                if (status != null)
                {
                    if (QuestManager.Instance.IsQuestCompleted(quest.QuestID))
                ...
                entry.GetComponent<Button>().onClick.AddListener(() => SelectQuest(capturedQuest));
            }
