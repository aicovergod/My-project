...
- **Companion System** (`Assets/Scripts/Companions`, `Assets/Scripts/Pets`)
  ...
  - Skill command surfaces (pet radial, AdminF4, dialogue proposals, scripted triggers) must funnel through `Companions.Commands.CompanionSkillCommandGuard.TryExecuteSingleTargetCommand` or `.TryExecuteAreaCommand`. The guard handles null/controller validation, guard-mode state, cooldown timers, and the shared inventory fallback hooks so every controller reports consistent log output and retry behaviour. Only invoke controllers directly when the guard already accepted the action to avoid desynchronised cooldown or inventory checks.
  ...
  - Gathering-focused controllers (`CompanionCookingController`, `CompanionFishingController`, `CompanionMiningController`, `CompanionWoodcuttingController`) now inherit from `CompanionSkillingControllerBase`. Initialise new skill controllers by calling `InitialiseMovementComponents()` and `ResetFollowerState()` so follower/path mover/pet animator references are cached consistently, and reuse `RebindPlayerSkill` to mirror how existing controllers reacquire player-side skills after respawns.
  - Whenever a controller needs to suspend the follower, increment the shared `followerDisableLockCount` exposed by `CompanionSkillingControllerBase` and surface a `HasActiveFollowerHold` property like the current skill controllers. Other systems must consult `CompanionController.HasActiveFollowerHold()` before re-enabling the follower to avoid breaking cooking/skill automation; do not bypass these locks by toggling `PetFollower` directly.
  ...
  - Bank deposits from companion menus should call `Companions.Inventory.CompanionBankDepositService.TryDepositCompanionInventoryToBank`. The helper resolves the active controller, validates nearby `BankSystem.CompanionBankDepositAnchor` components, emits chat barks, and logs debug output so UI flows (pet level bar, context menu, scripted banking) stay consistent. Place anchors on bank booths/chests and keep their radii in sync with scene design rather than duplicating range checks in UI code.
